...
export default function ReadChapterPage({ params }: ReadChapterPageProps) {
  ...
  // Lấy số chương từ params (loại bỏ tiền tố 'chapter-' nếu cần)
  const chapterNumber = parseInt(params.number.replace(/^chapter-/, ''), 10);

  // Tìm chapter theo số thay vì ID
  ...
  const prevChapter = manga.chapters.find(ch => ch.number === chapterNumber - 1) || null;
  const nextChapter = manga.chapters.find(ch => ch.number === chapterNumber + 1) || null;
        ...
        <div className="container py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {prevChapter ? (
                  <Link href={`/${manga.slug}/chapter-${prevChapter.number}`}>
              ...
              >
                {nextChapter ? (
                  <Link href={`/${manga.slug}/chapter-${nextChapter.number}`}>
